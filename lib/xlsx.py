...
def filt3_process(sn,c_f_m):
    ...
    if c_f_m == 'c':
        ...
        for i in ['GTinfo_','GT_','Qual_','Depth_']:
...
def xlsxgen(sn,ver,c_f_m):
    ...
    filt3.groupby(['Chr']).size().to_csv(sn.strip()+"_count", sep="\t", header=[sn])
    filt3.groupby(['DENOVO']).size().to_csv(sn.strip()+"_denovo", sep="\t", header=[sn])
    filt4.to_excel(wb,"splicing,stop gain loss,DD,DP",na_rep="-",index=False)
    if c_f_m == 'c_f_m':
        filt5 = filt5_process(filt3)
        #filt5.to_csv(sn+"_filt_selected_mutations", index=None, sep="\t", encoding='utf-8')
...
def xlsxgen_children_hospital(sn,ver,c_f_m):
    ...
    if c_f_m == 'c_f_m':
        filt5 = filt5_process(filt3)
        #filt5.to_csv(sn+"_filt_selected_mutations_children_hospital", index=None, sep="\t", encoding='utf-8')
        filt5.to_excel(wb,'filt_selected_mutations',na_rep = "-",index = False)
    readme.to_excel(wb,'readme',index = False)
    ws = wb.sheets['selected_mutations']
    for i in range(2,len(filt3)+1):
